...
def get_init_pos(bb, rnd):
    ...
    if rnd:
        return array([xmin + random() * (xmax - xmin), ymin + random() * (ymax - ymin)])
...
class Svgsort:
    ...
    def load(self, fn):
        paths, _, svg_atr = svg2paths(getcwd() + sep + fn, return_svg_attributes=True)
        self.paths = paths
    ...
    def repeat(self):
        self.paths.extend([attempt_reverse(deepcopy(p)) for p in reversed(self.paths)])
        self._repeat_report()
        return self
    ...
    def save_no_adjust(self, fn):
        ...
        disvg(
            ...
            svg_attributes=dict(
                {k: self.svg_atr[k] for k in keys if k in self.svg_atr}
            ),
        )
        print("wrote:", fn)
        return self
