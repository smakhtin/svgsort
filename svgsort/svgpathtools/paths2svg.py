...
def disvg(
    ...
    if nodes:
        if not node_colors:
            ...
            if isinstance(node_colors, str):
                node_colors = str2colorlist(
                    node_colors, default_color=_default_node_color
                )
    ...
    if svg_attributes is not None:
        dimensions = (
            svg_attributes.get("width", dimensions[0]),
        ...
        dwg = Drawing(
            filename=filename, size=dimensions, debug=svgwrite_debug, viewBox=viewbox
        )
