...
def bezier_point(p, t):
    ...
    if deg == 3:
        return p[0] + t * (
            3 * (p[1] - p[0])
            + t
            * (3 * (p[0] + p[2]) - 6 * p[1] + t * (-p[0] + 3 * (p[1] - p[2]) + p[3]))
        )
    elif deg == 2:
...
def bezier2polynomial(p, numpy_ordering=True, return_poly1d=False):
    ...
    if len(p) == 4:
        ...
        coeffs = [
            fac(n)
            // fac(n - j)
            * sum((-1) ** (i + j) * p[i] / (fac(i) * fac(j - i)) for i in range(j + 1))
            for j in range(n + 1)
        ]
        coeffs.reverse()
...
def polynomial2bezier(poly):
    ...
    if order == 3:
        ...
        raise AssertionError(
            "This function is only implemented for linear, "
...
def split_bezier(bpoints, t):
    ...
    bpoints_left = []
    bpoints_right = []
    bpoints_left, bpoints_right = split_bezier_recursion(
        bpoints_left, bpoints_right, bpoints, t
    )
...
def boxes_intersect(box1, box2):
    ...
    if interval_intersection_width(
        xmin1, xmax1, xmin2, xmax2
    ) and interval_intersection_width(ymin1, ymax1, ymin2, ymax2):
        return True
    else:
        return False
...
def bezier_intersections(bez1, bez2, longer_length, tol=1e-8, tol_deC=1e-8):
            ...
            if boxes_intersect(bbox1, bbox2):
                if box_area(*bbox1) < tol_deC and box_area(*bbox2) < tol_deC:
                    ...
                    for otherPair in pair_list:
                        if (
                            pair.bez1 == otherPair.bez1
                            or pair.bez2 == otherPair.bez2
                            or pair.bez1 == otherPair.bez2
                            or pair.bez2 == otherPair.bez1
                        ):
    ...
    if k >= maxits:
        raise Exception(
            "bezier_intersections has reached maximum "
